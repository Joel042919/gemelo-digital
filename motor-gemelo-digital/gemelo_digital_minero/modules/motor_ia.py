...
class MotorPredictivo:
    ...
    def entrenar_cnn_lstm(self):
        """Entrenar modelo híbrido CNN-LSTM"""
        if not TENSORFLOW_AVAILABLE:
            self._log("⚠️ TensorFlow no disponible, saltando...")
            return None
    ...
    def entrenar_lstm_ae_rf(self):
        """Entrenar modelo híbrido LSTM-Autoencoder + Random Forest"""
        if not TENSORFLOW_AVAILABLE:
            self._log("⚠️ TensorFlow no disponible, saltando...")
            return None
    ...
    def evaluar_algoritmo(self, nombre_algoritmo):
        ...
        if nombre_algoritmo == 'lstm_ae_rf':
            # Primero extraer características latentes
    ...
    def guardar(self, directorio=None):
        ...
        if hasattr(self, 'caracteristicas'):
            with open(os.path.join(dir_path, 'caracteristicas.json'), 'w') as f:
        ...
        if self.mejor_algoritmo:
            with open(os.path.join(dir_path, 'mejor_algoritmo.txt'), 'w') as f:
